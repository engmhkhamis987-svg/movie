...
class PopularComponentProv extends StatelessWidget {
  const PopularComponentProv({super.key});

  @override
  Widget build(BuildContext context) {
    return Consumer<MovieProvider>(
      builder: (context, value, child) {
        print("PopularComponentProv Build");
        switch (value.popularState) {
          ...
          case RequestState.error:
            return SizedBox(
              height: 170.0,
              child: Center(child: Text(value.popularMessage)),
            );
        }
      },
    );
